...
parser.add_argument('--model_path', dest='model_path', default='log/Gaussian4_newset_word2vec_act_1000dataset_attention_encoder_all_no_pretrain1628683592/020000000704320.model', type=str, help='载入模型位置')  #
...
def main():
    ...
    sigma1 = []
    for i in range(1, 5):
        checkpoint = torch.load('Gaussian_act_word2vec_1000data{}.model'.format(i))
        mu1.append(checkpoint['mu1'][0])
        sigma1.append(checkpoint['sigma1'][0])
    ...
    if os.path.isfile(args.model_path):  # 如果载入模型的位置存在则载入模型
        ...
        print('初始化模型完成')
        # 记录模型的文件夹
        log_dir = os.path.join(args.log_path, 'Gaussian4_newset_word2vec_act_1000dataset_attention_encoder_all_no_pre' + str(int(time.time())))
    ...
    if not args.inference:
        ...
        while epoch < args.max_epoch:  # 最大训练轮数
            ...
            for data in dp_train.get_batch_data():
                start_time = time.time()
                feed_data = prepare_feed_data(data)
                loss, nll_loss, kld_loss, ppl, kld_weight= train(model, feed_data, global_step,mu1[int(data['str_responses_act'][0][0])-1],sigma1[int(data['str_responses_act'][0][0])-1])
        ...
        result_file = os.path.join(args.result_path, 'cvae_{:03d}{:012d}.txt'.format(epoch, global_step))  # 命名结果文件
...
def compute_loss(outputs, labels, masks, global_step,mu1,sigma1):
    ...
    kld_loss2 = gaussian_kld(mu1,sigma1,mu,logvar)

    # 损失
    loss = nll_loss + kld_weight * kld_loss+kld_loss2
...
def train(model, feed_data, global_step,mu1,sigma1):
    ...
    loss, nll_loss, kld_loss, ppl, kld_weight = compute_loss(outputs, labels, masks, global_step,mu1,sigma1)  # 计算损失
    loss +=Loss
    return loss, nll_loss, kld_loss, ppl, kld_weight
