...
from model.model_visual_word2vec_attention_encoder_all import Model
...
parser.add_argument('--ingau', dest='ingau', default=True, type=bool, help='是否预训练初始化高斯')
parser.add_argument('--max_len', dest='max_len', default=60, type=int, help='测试时最大解码步数')
parser.add_argument('--model_path', dest='model_path', default='log/Gaussian4_newset_word2vec_act_1000dataset_attention_encoder_all_no_pretrain1628683592/060000002112960.model', type=str, help='载入模型位置')  #
...
def main():
    ...
    if os.path.isfile(args.model_path):  # 如果载入模型的位置存在则载入模型
        ...
        print('初始化模型完成')
        # 记录模型的文件夹
        log_dir = os.path.join(args.log_path, 'run_dialog_date' + str(int(time.time())))
    ...
    if True:
        ...
        dp_train = DataProcessor(trainset, config.batch_size, sentence_processor,shuffle=False)  # 数据的迭代器
        dp_valid = DataProcessor(validset+testset, config.batch_size, sentence_processor, shuffle=False)

        z, label = model(dp_train, word2vec, ingau=True, gpu=args.gpu)
        # np.save('z_encoder_all_06.npy', z)
        # np.save('z_encoder_all_20_label.npy', label)
        for i, data_ in enumerate(label):
            label[i][0] = int(sentence_processor.vocab[data_[0]])
        print(label)


        tsne = TSNE(n_components=2)
        z = tsne.fit_transform(z)
        plt.scatter(z[:, 0], z[:, 1], c=label, cmap='rainbow', s=5)
        plt.savefig("acvae-60epoch.jpg")

        # tsne = TSNE(n_components=3)
        # z2 = tsne.fit_transform(z)
        #
        # # 对数据进行归一化操作
        # x_min, x_max = np.min(z2, 0), np.max(z2, 0)
        # embedded = z2 / (x_max - x_min)
        # # 创建显示的figure
        #
        # fig = plt.figure()
        # ax = Axes3D(fig)
