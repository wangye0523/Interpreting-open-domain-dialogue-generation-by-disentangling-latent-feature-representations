...
from model.model_word2vec import Model
...
parser.add_argument('--validset_path', dest='validset_path', default='data/raw/trainset.txt', type=str, help='验证集位置')
...
parser.add_argument('--model_path', dest='model_path', default='log/rdel', type=str, help='载入模型位置')  #
...
def main():
    ...
    trainact1, trainact2, trainact3, trainact4 = [], [], [], []
    ...
    for data_act in trainset:
        if data_act['response_label_act'][0] == '1':
            trainact1.append(data_act)
        elif data_act['response_label_act'][0] == '2':
            trainact2.append(data_act)
        elif data_act['response_label_act'][0] == '3':
            trainact3.append(data_act)
        elif data_act['response_label_act'][0] == '4':
            trainact4.append(data_act)
    print(len(trainact1))
    print(len(trainact2))
    print(len(trainact3))
    print(len(trainact4))
    ...
    for num in range(1,5):
        ...
        if os.path.isfile(args.model_path):  # 如果载入模型的位置存在则载入模型
            ...
            print('初始化模型完成')
            # 记录模型的文件夹
            log_dir = os.path.join(args.log_path, 'act_pre_Gaussian_word2vec_145' + str(int(time.time())))
        ...
        if not args.inference:
            ...
            dp_train = DataProcessor(trainact[:1000], config.batch_size, sentence_processor)  # 数据的迭代器
            dp_valid = DataProcessor(validset[:1000], config.batch_size, sentence_processor, shuffle=False)
            ...
            if args.ingau:
                ...
                # model.save_model(0, 0, './GMM_act{}.model'.format(num))
            ...
            log_file = os.path.join(log_dir, 'Gaussian_act_word2vec_100_data_no_pre{}.model'.format(num))
...
def pre_train(model,feed_data):
    ...
    labels = feed_data['responses'][:, 1:]  # 去掉start_id
    masks = feed_data['masks']
    loss, nll_loss, kld_loss, ppl, kld_weight = compute_loss(outputs, labels, masks, 0)  # 计算损失
    return loss, nll_loss, kld_loss, ppl, kld_weight
